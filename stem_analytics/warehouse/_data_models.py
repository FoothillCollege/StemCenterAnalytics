...
def _create_data_path_mappings(dir_path: str, file_names: Iterable[str]) -> Dict[str, str]:
    ...
    for file_name in file_names:
        file_path = paths.join_path(dir_path, file_name)
        if paths.get_extension(file_path, with_dot=False) == 'sql':
            with database_io.connect_to_db(file_path):  # ensures valid connection (raises if issue)
                pass
        else:
            paths.ensure_file_path_exists(file_path, valid_file_types=['json', 'csv'])
...
def connect_to_stem_center_db() -> sqlite3.Connection:
    ...
    return database_io.connect_to_db(_INTERNAL_DATASETS_PATH_MAP['stem_center_db'])
...
def get_tutor_request_data(as_clean: bool=True) -> pd.DataFrame:
    ...
    if as_clean:
        with database_io.connect_to_db(_INTERNAL_DATASETS_PATH_MAP['stem_center_db']) as con:
            return database_io.read_database_table_as_df(con, 'tutor_requests')
    # otherwise read the tutor requests in from csv
    return file_io.read_flat_file_as_df(_EXTERNAL_DATASETS_PATH_MAP['unclean_tutor_requests'],
...
def get_student_login_data(as_clean: bool=True) -> pd.DataFrame:
    ...
    if as_clean:
        with database_io.connect_to_db(_INTERNAL_DATASETS_PATH_MAP['stem_center_db']) as con:
            return database_io.read_database_table_as_df(con, 'student_logins')
    # otherwise read the student logins in from csv
    return file_io.read_flat_file_as_df(_EXTERNAL_DATASETS_PATH_MAP['unclean_student_logins'])
    '''
    pass
